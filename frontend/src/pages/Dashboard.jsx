...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col bg-slate-50/50 pb-20">
      ...
      {/* 1. Dashboard Header */}
      <div className="w-full px-6 py-6 bg-white border-b border-slate-200 sticky top-0 z-10 flex flex-col sm:flex-row sm:items-start justify-between gap-4 shadow-sm">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight mb-2">Live Event Operations Center</h1>
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-slate-600">
            <span className="font-semibold text-slate-900">{eventData.name}</span>
            <span className="text-slate-300">|</span>
            <span>{eventData.venue || eventData.name}</span>
            <span className="text-slate-300">|</span>
            <span>{eventData.city}</span>
            <span className="text-slate-300">|</span>
            <span>{eventData.date}</span>
            <span className="text-slate-300">|</span>
            <span className="text-green-600 font-medium">Status: Live</span>
            <span className="text-slate-300">|</span>
            <span>Weather: Clear, 24°C</span>
          </div>
        </div>
        <div className="flex items-center space-x-3 bg-slate-50 px-4 py-2 rounded-lg border border-slate-200 h-fit">
          ...
          <span className="text-xs font-bold text-slate-700 tracking-wider uppercase">Live System Status: <span className={(wsStatus.includes('Connected') || wsStatus.includes('Simulated')) ? 'text-green-600' : 'text-red-600'}>{wsStatus}</span></span>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 hover:border-indigo-200 transition-colors">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-bold text-slate-500">Current Venue Occupancy</span>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 hover:border-indigo-200 transition-colors">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-bold text-slate-500">Expected Occupancy (Next 5 Minutes)</span>
            ...
            <div className="text-xs font-semibold text-indigo-500 mt-1">AI-powered forecast</div>
          </div>
